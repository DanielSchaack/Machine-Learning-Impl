...
public class PerceptronNandModel {
    ...
    public Perceptron train(int amountCoordinates, double standardDeviation, double learningRate, int batchSize) {
        ...
        showResults(dataPoints, standardDeviation, perceptron);
        return perceptron;
    }

    private HeatMapChart showHeatmap(double standardDeviation, Perceptron perceptron) {

        int rows = 101; 
        int cols = 101;
        List<Number> xData = generateIncrementingList(rows);
        List<Number> yData = generateIncrementingList(cols);
        ...
        for (int i = 0; i < xData.size(); i++) {
            ...
            for (int j = 0; j < yData.size(); j++) {
                ...
                double prediction = perceptron.predict(
                        PerceptronNandModel.getCoordinateDataPoint(x1, x2, standardDeviation));
    ...
    public static List<Number> generateIncrementingList(int totalPoints) {
...
private void showResults(List<DefaultDataPoint> dataPoints, double standardDeviation, Perceptron perceptron) {
        ...
        new SwingWrapper(List.of(chart, showHeatmap(standardDeviation, perceptron))).displayChartMatrix();
    }
}
